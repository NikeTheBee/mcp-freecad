...
| R3 | Graft clones tracked upstream `master` (`--depth 1`) | upstream API drift silently breaks `rocket_graft_test` / `drone_graft_test` | **Fixed** — `bootstrap.py` pins Rocket to `v5.1.1`; AirPlaneDesign tracks default branch (no stable tag — re-pin when one ships). |
